...
message. Rules are stored as arbitrary JSON, validated, and compiled into
...
- `headers`: map of unfolded header name to string value, with header names lowercased
...
Most subject, address, domain, header-contains, and attachment comparisons are
case-insensitive. Header names are lowercased. `headers_equals` compares the
normalized header value exactly after string conversion and trimming, so value
case and spacing matter. Leading/trailing whitespace is stripped by rule
validators. Unknown keys are accepted but ignored by the compiler, allowing
...
| `headers_equals` | `dict[str,str]` | Header name comparison is case-insensitive. Header value comparison is exact after normalization. |
| `headers_contains` | `dict[str,str]` | Header name comparison is case-insensitive. Header value must include the provided substring, also case-insensitively. Useful for prefixes like `"x-priority": "high"`. |
...
but exclude auto-replies" or "subject contains term *and* headers mention Plan X"
are expressible.
